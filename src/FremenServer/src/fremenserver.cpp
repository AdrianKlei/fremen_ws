...
void actionServerCallback(const fremenserver::FremenGoalConstPtr& goal, Server* as)
{
  ...
  {
    if (goal->times.size() == goal->values.size()){
      result.success = frelements.add_modified(goal->id.c_str(),(uint32_t*)goal->times.data(),(float*)goal->values.data(),(int)goal->values.size());
      if (result.success >=0)
      {
        mess << "Added " << result.success << " of the " << (int)goal->values.size() << " provided measurements to the state " << goal->id;
        result.message = mess.str();
  ...
  else if (goal->operation == "evaluate_modified")
  {
    if (goal->times.size() == goal->values.size()){
      float evaluations[goal->order+1];
      result.success = frelements.evaluate_modified(goal->id.c_str(),(uint32_t*)goal->times.data(),(float*)goal->values.data(),(int)goal->times.size(),goal->order,evaluations);
      if (result.success >= 0)
      {
        mess << "Performed " << (goal->order+1) << " evaluations of the model " << goal->id << " using " << (int)goal->times.size() << " ground truth values. The best performing model has order " << result.success;
        result.message = mess.str();
  ...
  else if (goal->operation == "predict_modified")
  {
    float probs[goal->times.size()];
    result.success = frelements.estimate_modified(goal->id.c_str(),(uint32_t*)goal->times.data(),probs,(int)goal->times.size(),goal->order);
    if (result.success >=0)
    {
      mess << "Performed " << (int)goal->times.size() << " predictions of the state " << goal->id;
      result.probabilities.assign(probs,probs + (int)goal->times.size());
...
int test()
{
	...
	float 		probsB[100000];
	int len = 7*24*60;
	FILE* file = fopen("/home/adrian/input.txt","r");
	int dummy=0;
	int dummy2=0;
	for (int i = 0;i<len;i++)
	{
...
int main(int argc,char* argv[])
{
	...
	server->start();
	ROS_INFO("fremenserver has been started...");
