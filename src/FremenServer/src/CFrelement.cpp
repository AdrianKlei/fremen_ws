...
CFrelement::CFrelement(const char* name)
{
	...
	// adro: what sense makes the 7*24*3600 as definition for the ground frequency ?
	for (int i=0;i<NUM_PERIODICITIES;i++) frelements[i].period = (7*24*3600)/(i+1); 
	gain = 0.5; // adro: what is the purpose of gain?
	firstTime = -1;
	lastTime = -1;
	measurements = 0;
}
...
int CFrelement::add(uint32_t times[],float states[],int length)
{
	...
	for (int i = 0;i<NUM_PERIODICITIES;i++)
	{
		...
		// adro: what sense makes this 1.5?
		if (1.0*frelements[i].period <= duration) frelements[i].amplitude = sqrt(re*re+im*im)/measurements; else frelements[i].amplitude = 0;
...
int CFrelement::add_modified(uint32_t times[],float states[],int length)
{
  if (measurements == 0 && length > 0)
  {
    for (int i = 0;i<NUM_PERIODICITIES;i++){
  ...
  //verify if ther is an actual update
  if (numUpdated <= 0)return numUpdated;
  lastTime = times[length-1];
  ...
  //recalculate spectral balance - this is beneficial is the process period does not math the length of the data
  if (oldGain > 0){
    for (int i = 0;i<NUM_PERIODICITIES;i++)
    {
      frelements[i].realBalance = gain*frelements[i].realBalance/oldGain;
...
//Adro : instead of unsigned char* signal, I should commit float values[]
int CFrelement::evaluate_modified(uint32_t* times,float* signal,int length,int orderi,float* evals)
{
  float estimate = 0;
  float time;
  float state;
  ...
  for (int j = 0;j<length;j++)
  {
    ...
    state = signal[j];
    estimate = gain;
    //Adro: here needs to be implemented the root mean squared error method! estimate needs to be rounded to the nearest integer value (STILL TO DO!)
    evals[0]+= pow(estimate-state, 2.0);
    for (int i = 0;i<orderi;i++){
      estimate+=2*frelements[i].amplitude*cos(time/frelements[i].period*2*M_PI-frelements[i].phase);
      evals[i+1]+= pow(estimate-state, 2.0);
    }
  }
  for (int j = 0;j<=orderi;j++)evals[j]= sqrt(evals[j]/length);

  //get best model order. No idea how to initialize float error. Maybe i have to change the way of computation?
  float error = 1000000;
  int index = 0;
  for (int j = 0;j<=orderi;j++)
  {
...
int CFrelement::estimate_modified(uint32_t times[],float probs[],int length,int orderi)
{
  float estimate = 0;
  float time;
  for (int j = 0;j<length;j++)
  {
    ...
    estimate = gain;
    for (int i = 0;i<orderi;i++) estimate+=2*frelements[i].amplitude*cos(time/frelements[i].period*2*M_PI-frelements[i].phase);
    //Adro: here needs to be implemented the estimate functionality
    //Adro: do I need to implement an upper boundary as well?
    if (estimate < 0.0) estimate = 0.0;
    probs[j]=estimate;
  }
  return length;
